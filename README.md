...
1. Пользователь взаимодействует с представлением (например, нажимает кнопку "Добавить в корзину").
2. Презентер (`BasketPresenter`) вызывает метод `add` у экземпляра `Basket`.
3. `Basket` обновляет массив `items`.
4. Презентер вызывает метод `updateBasket` у представления (`BasketView`), обновляя отображение корзины.

#### Пример взаимодействия при добавлении товара

1. Пользователь нажимает кнопку "Добавить в корзину".
2. `BasketPresenter` вызывает метод `add` у экземпляра `Basket`.
3. `Basket` обновляет массив `items` и уведомляет презентер.
4. `BasketPresenter` вызывает метод `updateBasket` у `BasketView`, который обновляет интерфейс.
