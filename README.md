...
Класс `ShoppingCart` представляет корзину покупок в приложении. Он управляет товарами, находящимися в корзине, вычисляет общую стоимость и предоставляет методы для добавления, удаления и очистки товаров в корзине. Также он отвечает за оповещение других частей приложения о изменениях в корзине.

## Свойства

- **`_goods`**: Массив товаров (`IProductItem[]`), находящихся в корзине. Доступ к этому свойству осуществляется через геттер и сеттер.
- **`total`**: Общая стоимость всех товаров в корзине.
...
Обновляет общую стоимость корзины и вызывает метод `checkEmpty()` для проверки, пуста ли корзина. Вызывает событие `basketData:changed` с информацией о добавленном товаре.
...
Обновляет общую стоимость корзины и вызывает метод `checkEmpty()` для проверки, пуста ли корзина. Вызывает событие `basketData:changed` с информацией о удалённом товаре.

### `clearBasket()`

Очищает корзину, устанавливая массив товаров в пустой.

Обновляет общую стоимость корзины и вызывает метод `checkEmpty()` для проверки, пуста ли корзина. Вызывает событие `basketData:changed` с пустым массивом товаров.
...
- Общую стоимость (`number`). Стоимость вычисляется динамически на основе товаров в корзине.
...
Класс `Order` представляет заказ в приложении. Он управляет данными о заказе, такими как метод оплаты, контактная информация пользователя и данные корзины. Класс также предоставляет методы для получения полной информации о заказе.

## Свойства

- **`_payment`**: Метод оплаты (`PaymentMethod`). Приватное свойство, которое хранит метод оплаты для заказа.
- **`_email`**: Электронная почта пользователя. Приватное свойство, которое хранит электронную почту пользователя.
- **`_address`**: Адрес пользователя. Приватное свойство, которое хранит адрес пользователя.
- **`_phone`**: Телефонный номер пользователя. Приватное свойство, которое хранит телефонный номер пользователя.
- **`cart`**: Экземпляр класса `ShoppingCart`, содержащий данные о товарах в корзине. Приватное свойство.
- **`events`**: Экземпляр класса `IEvents`, используемый для управления событиями.
...
Создаёт экземпляр заказа с заданным объектом `events` и экземпляром корзины `cart`.

**Параметры:**

- **`events`**: Экземпляр класса `IEvents`, используемый для управления событиями.
- **`cart`**: Экземпляр класса `ShoppingCart`, содержащий данные о товарах в корзине.
...
Возвращает общую сумму заказа, полученную из корзины.

**Возвращает:**

- Общую сумму заказа (`number`), вычисленную на основе данных в корзине.

### `get items(): string[]`

Возвращает массив идентификаторов товаров, полученный из корзины.

**Возвращает:**

- Массив идентификаторов товаров (`string[]`), находящихся в корзине.

### `get orderFullInfo(): object`

Возвращает полную информацию о заказе, включая метод оплаты, электронную почту, телефонный номер, адрес, общую сумму и массив товаров.

**Возвращает:**

- Объект с полной информацией о заказе, который включает:
  - Метод оплаты (`PaymentMethod`).
  - Электронную почту пользователя (`string`).
  - Телефонный номер пользователя (`string`).
  - Адрес пользователя (`string`).
  - Общую сумму заказа (`number`).
  - Массив идентификаторов товаров (`string[]`).
...
- **`ShoppingCart`**: Класс, представляющий корзину покупателя, используется для получения данных о товарах и их общей стоимости.
