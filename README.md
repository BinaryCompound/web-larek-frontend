...
- `_goods`: Приватное свойство. Массив товаров (`IProductItem[]`), находящихся в корзине. Доступ к этому свойству осуществляется через геттер и сеттер.
- `events`: Экземпляр класса `IEvents`. Используется для управления событиями, связанными с корзиной.
...
- Текущий массив товаров (`IProductItem[]`).

#### `set goods(items: IProductItem[]): void`

Сеттер для установки массива товаров в корзине. При изменении товаров вызывается метод `checkEmpty()`, который проверяет, пуста ли корзина.
...
Проверяет, есть ли товар в корзине по идентификатору `id`. Если товар присутствует, вызывается событие `addToBasket:disabled`.
...
Добавляет товар в корзину. После добавления товара вызывается метод `checkEmpty()` для проверки, пуста ли корзина, и событие `basketData:changed` с информацией о добавленном товаре.
...
Удаляет товар из корзины по идентификатору `id`. После удаления товара вызывается метод `checkEmpty()` для проверки, пуста ли корзина, и событие `basketData:changed` с информацией о удалённом товаре.
...
Очищает корзину, устанавливая массив товаров в пустой. После очистки корзины вызывается метод `checkEmpty()` для проверки, пуста ли корзина, и событие `basketData:changed` с пустым массивом товаров.
...
Возвращает общую стоимость всех товаров в корзине, вычисленную динамически на основе текущих товаров в корзине.

**Возвращает:**

- Общую стоимость (`number`).
...
Возвращает объект с полной информацией о заказе, включая способы оплаты, контактные данные, адрес и информацию о товарах.
...
- Объект с полной информацией о заказе.

#### `private checkEmpty(): void`

Приватный метод, проверяющий, пуста ли корзина. Если корзина пуста, вызывается событие `basket:empty`. Если в корзине есть товары, вызывается событие `basket:has-items`.

## Зависимости

- `IBasketData`: Интерфейс, который реализует класс `ShoppingCart`.
- `IProductItem`: Интерфейс, описывающий свойства товара.
...
Класс `Order` представляет заказ в системе и управляет такими данными, как метод оплаты, контактная информация и адрес пользователя. Класс предоставляет геттеры и сеттеры для управления этими данными.
...
- `events`: Публичное свойство, представляющее экземпляр интерфейса `IEvents`, используемое для управления событиями.
...
- `IOrderData`: Интерфейс, который реализует класс `Order`.
- `PaymentMethod`: Тип, определяющий методы оплаты (`'cash' | 'card'`).
- `IEvents`: Интерфейс для работы с событиями, используется для оповещения о различных действиях и изменениях.
